...
class InputData2:
    ...
    @classmethod
    def from_search_node(cls, node: TableauSearchNode) -> "InputData2":
    ...
    def get_encoding_tensor(self, word_encoder: WordEncoder) -> torch.Tensor:
        """Return a tensor E x 3 where E is the number of events.
        Every row has the tuple <agent, type> encoding"""
        output = torch.zeros(
            (len(self.events_data), 1 + 2 * word_encoder.word_encoding_length),
            dtype=torch.float32,
            device=word_encoder.device,
        )

        for e, event_data in enumerate(self.events_data):
            agent_encoding = -reduce(
                operator.add,
                map(
                    lambda f: word_encoder.encode_word(f.formula.agent.name),
                    event_data.negative_agents,
                ),
                0,
            ) / max(1, len(event_data.negative_agents))
            positive_agents = list(event_data.positive_agents)
            if positive_agents:
                agent_encoding = word_encoder.encode_word(positive_agents[0].agent.name)
            output[e, : word_encoder.word_encoding_length] = agent_encoding
            type_encoding = -reduce(
                operator.add,
                map(
                    lambda f: word_encoder.encode_word(f.formula.type_.name),
                    event_data.negative_types,
                ),
                0,
            ) / max(1, len(event_data.negative_types))
            positive_types = list(event_data.positive_types)
            if positive_types:
                type_encoding = word_encoder.encode_word(positive_types[0].type_.name)
            output[e, word_encoder.word_encoding_length : -1] = type_encoding
            output[e, -1] = self.model_depth
        return output


@dataclass
class GRUModelConfig2:
...
class GRUModel2(torch.nn.Module, Heuristic):
    """Deep learning model.
    This model uses a linear feature extraction block,
    followed by an LSTM block, then a prediction head for the score output
    """

    def __init__(self, cfg: Optional[GRUModelConfig2] = None):
        super().__init__()
        if cfg is None:
            cfg = GRUModelConfig2()
        self._cfg = cfg
        self.encoder = WordEncoder(self._cfg.device)
        self.event_encoding_length = self.encoder.word_encoding_length * 2 + 1
    ...
    def _build_feature_extraction_backbone(self, input_size: int) -> torch.nn.Module:
        layers = [
            torch.nn.Linear(input_size, self._cfg.latent_size),
            torch.nn.ReLU(),
    ...
    def _make_empty_branch_tensor(self, _: InputData2) -> torch.Tensor:
        """Create an input tensor that represents a model with no new event information."""
        return torch.zeros(
            (1, self.event_encoding_length),
            dtype=torch.float32,
    ...
    def _from_search_node(
        ...
        """
        input_data: InputData2 = InputData2.from_search_node(node)
        encoding: torch.Tensor = input_data.get_encoding_tensor(self.encoder)
    ...
    def forward_batch(self, xs: List[torch.Tensor], h0s: torch.Tensor | None = None):
        ...
        flat_features: torch.Tensor = self.feature_extraction_backbone(
            flat_sequences
        )  # Batch Size * Sequence Length x LSTM Input Shape
        ...
        lstm_output_features, lstm_hidden_output = self.memory_unit(
            packed_feature_tensor, h0s
        )
        # flatten the lstm output features to pass onto the prediction head
        unpacked_lstm_features = torch.nn.utils.rnn.unpack_sequence(
    ...
    def get_node_encoding(self, node: TableauSearchNode) -> torch.Tensor:
