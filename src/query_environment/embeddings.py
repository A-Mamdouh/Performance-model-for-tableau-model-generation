...
class EventInformationManager:
    ...
    def get(self, key: Term) -> Optional[Tuple[EventInformation, EventEmbedding]]:
        ...
        embd = EventEmbedding(self, None, None, None, None, None) # TODO: Calculate these values
    ...
    def copy(self, **changes) -> "EventInformationManager":
        ...
        copy_ = EventInformationManager(
            events_information= { event: info.copy() for event, info in self.events_information.items()},
            events_embeddings= { event: embd.copy() for event, embd in self.events_embeddings.items()},
        )
